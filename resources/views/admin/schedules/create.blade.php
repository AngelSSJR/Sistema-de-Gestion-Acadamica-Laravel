...
<div class="fade-in">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold mb-0">
            <i class="bi bi-calendar-plus text-info me-2"></i>Nuevo Horario
        </h2>
    ...
    <div class="card shadow-sm">
        <div class="card-body p-4">
            <form method="POST" action="{{ route($routePrefix . 'schedules.store') }}">
                @csrf
                ...
                <div class="row g-3">
                            ...
                            @foreach(['monday' => 'Lunes', 'tuesday' => 'Martes', 'wednesday' => 'Miércoles', 'thursday' => 'Jueves', 'friday' => 'Viernes', 'saturday' => 'Sábado'] as $val => $label)
                                <div class="form-check">
                                    <input class="form-check-input @error('day_of_week') is-invalid @enderror" type="checkbox"
                                           ...
                                           @checked(is_array(old('day_of_week')) && in_array($val, old('day_of_week')))>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="time" class="form-control @error('end_time') is-invalid @enderror" id="end_time" name="end_time" value="{{ old('end_time', '08:00') }}" required>
                    ...
                    <div class="col-md-3">
                        ...
                        <select class="form-select @error('room_id') is-invalid @enderror" id="room_id" name="room_id">
                            ...
                            @foreach($rooms as $room)
                                <option value="{{ $room->id }}" @selected(old('room_id') == $room->id)>{{ $room->name }} ({{ $room->code }})</option>
                            @endforeach
                        </select>
